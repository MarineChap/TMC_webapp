...
async function addLog(action, metadata = {}) {
    try {
        let logs = [];
        try {
...
app.post('/api/delete', async (req, res) => {
    try {
        ...
        const itemIndex = dbData[category].findIndex((dbItem) => JSON.stringify(dbItem) === JSON.stringify(item));
        if (itemIndex === -1) {
...
app.get('/api/sdmis-rss', async (req, res) => {
    try {
        ...
        if (!response.ok)
            throw new Error('Failed to fetch SDMIS feed');
